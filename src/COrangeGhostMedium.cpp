/*
 * @author Šimon Matouš (matousi1)
 * on 13.06.2020
 */
#include "COrangeGhostMedium.h"

void COrangeGhostMedium::Stun()
{
    m_roundsLeftStunned = 20;
}

void COrangeGhostMedium::AddWait()
{
    m_waitTillSpawn = 15;
}

char COrangeGhostMedium::Strategy(const char & /* dir */, std::map<std::pair<size_t, size_t>, char> &m_Content,const size_t & x,const size_t & y)
{
    if ( m_speedIterator == m_speed ) {
        int holder;
        char retVal;
        holder = Convertor(m_X, m_Y, x, y);
        /** determines which mode is ghost in */
        if ((m_modeCounter >= 10 || m_switchToChase > 3) && m_roundsLeftStunned == 0) {
            if (holder < 12)
                retVal = ScatterChase(4, 3, 2, 1, m_Content, x, y);
            else {
                size_t scatter_Y = m_Content.rbegin()->first.first;
                retVal = ScatterChase(4, 3, 2, 1, m_Content, 0, scatter_Y);
            }
        } else {
            size_t scatter_Y = m_Content.rbegin()->first.first;
            retVal = ScatterChase(4, 3, 2, 1, m_Content, 0, scatter_Y);
        }
        ModeManager();
        m_speedIterator = 0;
        return retVal;
    }
    else {
        m_speedIterator++;
        return 'D';
    }
}
void COrangeGhostMedium::ModeManager()
{
    if ( m_roundsLeftStunned > 0 )
        m_roundsLeftStunned--;
    else {
        if (m_modeCounter == 10)
            ChangeDir();
        if (m_modeCounter == 30) {
            m_modeCounter = 0;
            m_switchToChase++;
            ChangeDir();
        } else
            m_modeCounter++;
    }
}